...
import type { QuantumNeuralFormationIndex } from "./types";

export class GraphNode {
  position: THREE.Vector3;
  connections: Array<{ node: GraphNode; strength: number }>;
  level: number;
  type: number;
  size: number;
  ...
  constructor(position: THREE.Vector3, level = 0, type = 0) {
    ...
    this.type = type;
    this.size =
      type === 0 ? THREE.MathUtils.randFloat(0.8, 1.4) : THREE.MathUtils.randFloat(0.5, 1.0);
    this.distanceFromRoot = 0;
  }

  addConnection(node: GraphNode, strength = 1.0) {
  ...
  isConnectedTo(node: GraphNode) {
    return this.connections.some((conn) => conn.node === node);
  }
}

export const COLOR_PALETTES: THREE.Color[][] = [
  [
...
export type GeneratedNetwork = { nodes: GraphNode[]; rootNode: GraphNode };
...
export function generateNeuralNetwork(
  ...
  function generateCrystallineSphere() {
    rootNode = new GraphNode(new THREE.Vector3(0, 0, 0), 0, 0);
    rootNode.size = 2.0;
    nodes.push(rootNode);
    const layers = 5;
    ...
    for (let layer = 1; layer <= layers; layer++) {
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        const node = new GraphNode(pos, layer, isLeaf ? 1 : 0);
  ...
  function generateHelixLattice() {
    ...
    for (let h = 0; h < numHelices; h++) {
      ...
      for (let i = 0; i < nodesPerHelix; i++) {
        ...
        const node = new GraphNode(pos, level, isLeaf ? 1 : 0);
      ...
      for (let i = 0; i < helix.length; i += 8) {
        ...
        if (innerNodes.length > 0) {
          const nearest = innerNodes.sort(
            (a, b) => node.position.distanceTo(a.position) - node.position.distanceTo(b.position),
          )[0];
  ...
  function generateFractalWeb() {
    ...
    const branches = 6;
    const maxDepth = 4;
    function createBranch(startNode: GraphNode, direction: THREE.Vector3, depth: number, strength: number, scale: number) {
      ...
      const newNode = new GraphNode(endPos, depth, isLeaf ? 1 : 0);
  ...
  if (densityFactor < 1.0) {
    ...
    const toKeep = new Set<GraphNode>([rootNode]);
