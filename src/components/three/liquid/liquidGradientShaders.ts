/** Port of SPECIAL/interactive-liquid-gradient-using-three-js (CodePen) — GLSL 1.0 for ShaderMaterial. */
...
export const LIQUID_FRAGMENT_SHADER = /* glsl */ `
  ...
  vec3 getGradientColor(vec2 uv, float time) {
    ...
    float mixFactor1 = max(brightness1 * 1.15, 0.18);
  ...
  void main() {
    ...
    uv.x += vx * 0.55 * touchIntensity;
    uv.y += vy * 0.55 * touchIntensity;
    vec2 c = vec2(0.5);
    float d = length(uv - c);
    float ripple = sin(d * 20.0 - uTime * 3.0) * 0.03 * touchIntensity;
    float wave = sin(d * 15.0 - uTime * 2.0) * 0.025 * touchIntensity;
    uv += vec2(ripple + wave);
    ...
    float mixFactor2 = max(brightness2 * 1.1, 0.18);
